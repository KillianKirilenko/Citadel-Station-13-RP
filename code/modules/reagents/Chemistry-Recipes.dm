...
/proc/initialize_chemical_reactions()
	...
	SSchemistry.chemical_reactions_list = list()

	for(var/path in paths)
		var/datum/chemical_reaction/D = new path()
		if(D.required_reagents && D.required_reagents.len)
			var/reagent_id = D.required_reagents[1]
			if(!SSchemistry.chemical_reactions_list[reagent_id])
				SSchemistry.chemical_reactions_list[reagent_id] = list()
			SSchemistry.chemical_reactions_list[reagent_id] += D
